...
for VERSION in "${VERSIONS[@]}"; do
    ...
    if [ -x "detect" ]; then
        ...
        for IMG_PATH in "${IMAGES[@]}"; do
            ...
            echo "  -> Procesando imagen: $IMG_NAME"
            
            # Ejecutar detect con time redirigiendo todo el output para capturarlo
            OUTPUT=$(/usr/bin/time -p ./detect "$IMG_PATH" 2>&1)
